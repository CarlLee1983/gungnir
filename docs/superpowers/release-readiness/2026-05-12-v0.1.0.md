...
./scripts/lint   # explicit ShellCheck verification
```
...
Summary output from `./scripts/release-check all` on 2026-05-12:
...
- No `v0.1.0` GitHub Release has been published from this work. Tagging and
  ...
  future spec.
- ShellCheck ran on this pass (version 0.11.0). For future passes on machines
  where ShellCheck is not installed, `./scripts/lint` will skip the check —
  re-run on a machine with ShellCheck before tagging.
